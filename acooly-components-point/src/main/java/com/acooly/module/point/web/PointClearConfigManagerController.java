...
public class PointClearConfigManagerController
		extends AbstractPointManageController<PointClearConfig, PointClearConfigService> {
	...
	@ResponseBody
	public JsonEntityResult<PointClearConfig> clearJson(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String startTime = Dates.format(pointClearConfig.getStartClearTime(), Dates.CHINESE_DATETIME_FORMAT_LINE);
			String endTime = Dates.format(pointClearConfig.getEndClearTime(), Dates.CHINESE_DATETIME_FORMAT_LINE);
